...
@Component({
  ...
  standalone: true, 
  imports: [MatFormFieldModule, MatSelectModule, MatInputModule, PrimaryButtonComponent, ReactiveFormsModule, CommonModule], 
  template: `
   <div class="form-container">
      <form [formGroup]="studentGradeForm" (ngSubmit)="onSubmit()">

      
       <button type="button" class="back-button" (click)="goBack()">Back to Home</button>
       ...
       <input type="hidden" formControlName="studentGradeId">
        <div class="form-row">
          <mat-form-field appearance="outline">
            ...
            <mat-error *ngIf="studentGradeForm.get('firstName')!.hasError('required')">First Name is required</mat-error>
          </mat-form-field>

          <mat-form-field appearance="outline">
            <mat-label>Last Name</mat-label>
            <input matInput placeholder="Enter last name" formControlName="lastName" required />
            <mat-error *ngIf="studentGradeForm.get('lastName')!.hasError('required')">Last Name is required</mat-error>
          </mat-form-field>
        </div>
        ...
        <div class="form-row">
          <mat-form-field appearance="outline">
            ...
            <mat-select formControlName="subjectId" required>
              <mat-option *ngFor="let subject of subjects" [value]="subject.subjectId">{{ subject.subjectName }}</mat-option>
            </mat-select>
            <mat-error *ngIf="studentGradeForm.get('subjectId')!.hasError('required')">Subject is required</mat-error>
          </mat-form-field>
          ...
          <mat-form-field appearance="outline">
            ...
            <mat-error *ngIf="studentGradeForm.get('grade')!.hasError('required')">Grade is required</mat-error>
          </mat-form-field>
        </div>
        ...
        <div class="form-actions">
          <app-primary-button label="Submit" [disabled]="studentGradeForm.invalid || isSubmitSuccessful"></app-primary-button>
        </div>
      </form>
    </div>
  `,
...
export class StudentgradingformComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (studentGradeId) {
      ...
      this.studentGradeService.GetStudentGradeById(studentGradeId).subscribe((response) => {
  ...
  onSubmit(): void {
    if (this.studentGradeForm.valid) {
      ...
      if (this.isEditMode) {
        this.studentGradeService.UpdateStudentGrade(studentGrade).subscribe((response) => {
          if (response.success) {
        ...
        this.studentGradeService.AddStudentGrade(studentGrade).subscribe((response) => {
          if (response.success) {
            this.isSubmitSuccessful = true;
            this.router.navigate(['/']);
          } else {
